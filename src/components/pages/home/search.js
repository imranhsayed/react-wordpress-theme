...
const Search = ( props ) => {
	...
	const backgroundURL = searchSection.searchBackURL
		? searchSection.searchBackURL
		: config.searchSection.searchBackURL;
	const placeholderText = searchSection.searchPlaceholderTxt
		? searchSection.searchPlaceholderTxt
						...
						>
							{ term.image.img_srcset ? (
								<div className="search-section__category-icon">
									<img
										src={ term.image.img_src[ 0 ] }
										alt={ term.name }
										srcSet={ term.image.img_srcset }
									/>
								</div>
							) : (
								<div className="search-section__category-icon">
